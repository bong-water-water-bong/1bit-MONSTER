        ...
        // boundaries).
        {
            ...
            for (int r = 0; r < R; r++)
                for (int g = 0; g < C / 32; g++) {
                    float zg = i4p_bf16_to_f32(pg.zp_g_bf16[(size_t)r * (C / 32) + g]);
                    double d = std::fabs((double)zg - t.zp[(size_t)r * (C / 32) + g]);
                    if (d > zp_maxd) zp_maxd = d;
                    if (d > 1e-6) zp_bad++;
                }
            CHECK(zp_bad == 0 && zp_maxd <= 1e-6,
                  "%s: zp grid byte-exact vs raw (folded) zp: %ld/%ld bad, maxdiff=%.8f",
                  tname, zp_bad, (long)R * (C / 32), zp_maxd);
            ...
            CHECK(dz_bad * 100000 <= dz_tot,   // <= 0.001% round-boundary diff
