...
// WMMA-accelerated ternary GEMV — uses __builtin_amdgcn_wmma_i32_16x16x16_iu8_w32
// for 16× higher arithmetic density vs DP4A sudot4. Falls back to DP4A halo_f16
// when K < 16 or K % 16 != 0. Output is FP16 (fused convert).
// Weights: packed 2-bit halo format (same as rcpp_ternary_gemv_halo_f16).
// K must be a multiple of 16 for WMMA path. FP16 output.
rcpp_status_t
rcpp_ternary_gemv_wmma(const void* packed_weights_dev,
                       const void* activations_i8_dev,
                       float       activation_scale,
                       const void* row_scales_dev,
                       void*       output_f16_dev,
...
// Fused RMSNorm + INT8 quantize in one kernel pass.
// Computes: normed = x * inv_rms * weight, then quantizes to INT8 on the fly.
// Saves 1 kernel launch + 1 full buffer write vs calling both separately.
// Weight is RMSNorm weight; bias is NOT applied (residual uses the original).
rcpp_status_t
rcpp_rmsnorm_quant_i8(const void* x_dev, const void* weight_dev,
                      void* x_i8_dev, float* scale_dev,
                      float eps, int K, void* stream);
...
// Fused: RoPE Q + K + write K/V to KV cache in ONE kernel launch.
// x_dev has Q heads [num_q_heads, head_dim] then K heads [num_kv_heads, head_dim].
// Replaces: rcpp_rope_fp16(Q) + rcpp_rope_fp16(K) + hipMemcpy(K→cache) +
//           hipMemcpy(V→cache). Saves 3 kernel launches per layer.
rcpp_status_t
rcpp_rope_kv_append_fp16(
    void* x_dev,              // [NH+NKV, HD] Q then K, in-place RoPE'd
    void* k_cache_dev,        // [max_seq, NKV, HD] destination for RoPE'd K
    const void* v_src_dev,    // [NKV, HD] V source (no RoPE needed)
    void* v_cache_dev,        // [max_seq, NKV, HD] destination for V
    int pos, float theta,
...
// Fused Residual Add + RMSNorm: x_fp32 += src_fp16, then y = norm(x) * weight.
// Replaces residual_add_fp32_from_fp16() + rmsnorm_fp32_in_fp16_out() sequence.
// Saves 1 full buffer read/write (FP32 x read + x write combined with norm pass).
rcpp_status_t
rcpp_resadd_rmsnorm_fp32_out(void* x_fp32_dev, const void* src_fp16_dev,
                             const void* weight_dev, void* y_fp16_dev,
                             float eps, int K, void* stream);
...
rcpp_status_t
rcpp_fp16_to_fp32(const void* x_fp16_dev, void* y_fp32_dev, int N, void* stream);
...
rcpp_status_t rcpp_hadamard_rotate_fp16(
    const void* x_in, void* y_out, int K, void* stream);

// Low-level butterfly launcher (void return, pre-checked K).
// Prefer rcpp_hadamard_rotate_fp16 for status-returning wrapper.
void rcpp_hadamard_rotate_fp16_butterfly_launch(
    const void* x_in, void* y_out, int K, void* stream);

// -----------------------------------------------------------------------------
// WMMA INT8×INT8 GEMV — Hadamard-rotated decode.
//
// Weights: INT8 [M, K] — Hadamard-rotated at export time.
// Activation: INT8 [K] — Hadamard-rotated then quantized at runtime.
// Compute: wmma_i32_16x16x16_iu8 — 2× throughput vs FP16 WMMA.
// Output: FP16 [M] with per-row fp16 scale.
//
// K must be a multiple of 16 (WMMA tile dimension).
rcpp_status_t rcpp_wmma_i8_gemv(
